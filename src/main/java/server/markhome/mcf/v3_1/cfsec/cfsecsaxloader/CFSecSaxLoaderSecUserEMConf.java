...
/*
 *	CFSecSaxLoaderSecUserEMConfParse XML SAX Element Handler implementation
 *	for SecUserEMConf.
 */
public class CFSecSaxLoaderSecUserEMConf
	extends CFLibXmlCoreElementHandler
{
	public CFSecSaxLoaderSecUserEMConf( CFSecSaxLoader saxLoader ) {
		super( saxLoader );
	}
	...
	throws SAXException
	{
		...
		ICFSecSecUserEMConfObj origBuff = null;
		ICFSecSecUserEMConfEditObj editBuff = null;
		...
		try {
			...
			origBuff = (ICFSecSecUserEMConfObj)schemaObj.getSecUserEMConfTableObj().newInstance();
			editBuff = (ICFSecSecUserEMConfEditObj)origBuff.beginEdit();
			...
			CFSecSaxLoader.LoaderBehaviourEnum loaderBehaviour = saxLoader.getSecUserEMConfLoaderBehaviour();
			ICFSecSecUserEMConfEditObj editSecUserEMConf = null;
			ICFSecSecUserEMConfObj origSecUserEMConf = (ICFSecSecUserEMConfObj)schemaObj.getSecUserEMConfTableObj().readSecUserEMConfByUUuid6Idx( editBuff.getRequiredEMConfirmationUuid6() );
			if( origSecUserEMConf == null ) {
				...
				switch( loaderBehaviour ) {
					case Insert:
						break;
					case Update:
						editSecUserEMConf = (ICFSecSecUserEMConfEditObj)origSecUserEMConf.beginEdit();
			...
			if( editSecUserEMConf != null ) {
				if( origSecUserEMConf != null ) {
					...
					origSecUserEMConf = (ICFSecSecUserEMConfObj)editSecUserEMConf.create();
				}
				editSecUserEMConf = null;
			}
